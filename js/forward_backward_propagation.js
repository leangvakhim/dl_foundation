...
// Definition of our interactive steps
const steps = [
    {
    ...
    },
    {
        ...
        state: {
            activeNodes: ['n-i1', 'n-i2', 'n-h1', 'n-h2'],
            forwardEdges: elements.edges.i_h,
            backwardEdges: []
        }
    },
    {
        ...
        state: {
            activeNodes: ['n-h1', 'n-h2', 'n-o1'],
            forwardEdges: elements.edges.h_o,
            backwardEdges: []
        }
    },
    {
        ...
        state: {
            activeNodes: ['n-o1'], // Target state handled via CSS later
            forwardEdges: [],
            backwardEdges: []
        }
    },
    {
        ...
        state: {
            activeNodes: ['n-o1', 'n-h1', 'n-h2'],
            forwardEdges: [],
            backwardEdges: elements.edges.h_o
        }
    },
    {
        ...
        state: {
            activeNodes: ['n-h1', 'n-h2', 'n-i1', 'n-i2'],
            forwardEdges: [],
            backwardEdges: elements.edges.i_h
        }
    },
    {
    ...
    },
    {
        ...
        math: "", // Handled separately in the summary div
        state: {
            activeNodes: ['n-i1', 'n-i2', 'n-h1', 'n-h2', 'n-o1'],
...
function resetSVG() {
    ...
    document.querySelectorAll('.node').forEach(el => {
        el.className.baseVal = 'node node-inactive';
    });
    // Reset edges
    document.querySelectorAll('.edge').forEach(el => {
        el.className.baseVal = 'edge edge-inactive';
    });
}
...
function renderMath(step) {
    ...
    if (step.type === 'equations') {
        // Show the large summary, hide the small math block
        mathContainer.classList.add('hidden');
        summaryContainer.classList.remove('hidden');

        // Render the summary equations once
        ...
        // Show small math block, hide summary
    ...
    // Simple regex to find $math$ and render it inline
...
function updateUI() {
    ...
    // Adjust badge colors based on flow direction
    ...
    steps.forEach((_, index) => {
        ...
        dot.className = `w-2 h-2 rounded-full transition-colors duration-300 ${index === currentStep ? 'bg-blue-600' : 'bg-slate-300'}`;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof katex !== 'undefined') {
        ...
        // Fallback loop if CDN is slightly delayed
